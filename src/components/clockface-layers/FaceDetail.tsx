import { For, Index, Show, createEffect, createMemo, on, onCleanup, type Component } from "solid-js";
...
/**
 * 時計盤のうち「地の上に乗る読み取り要素」レイヤー。区切りの色扇・12/3/6/9 境界線・バッジ円・分目盛り・
 * monotone-badge の縁メモリ・くわしくの分数字・時間数字 (12h⇄24h バウンス含む) を描く SVG フラグメント。
 * ベゼル/背景色は BaseFace、針は HandsLayer。タイマー扇はこのレイヤーの「下」(BaseFace との間) に入る。
 */
...
const FaceDetail: Component<FaceDetailProps> = (props) => {
  ...
  return (
    <>
       ...
       *  外端を outerRing (R+3) まで延伸して外側リングに沿わせ、外側 cap は flat (butt)。
      ...
      <Show when={isMonotoneBadge()}>
        <For each={Array.from({ length: 60 })}>
          {(_, idx) => {
            ...
            const inner = () => isMajor() ? clockRadius() - 12 : clockRadius() - 5;
            const sw = () => isMajor() ? 3 : 1.8;
            const ix = () => CENTER + inner() * Math.cos(angle());
            const iy = () => CENTER + inner() * Math.sin(angle());
            return (
              <g>
                <line
                  x1={ix()} y1={iy()}
      ...
      <Index each={POSITIONS}>
        {(_pos, position) => {
          ...
          const effectiveR = () => numberRadius() + radialNudge();
          const x = () => CENTER + effectiveR() * Math.cos(angle());
          const y = () => CENTER + effectiveR() * Math.sin(angle());
      ...
      <Show when={props.period === "pm"}>
        <TimeFormatPrerollFx centerX={CENTER} centerY={CENTER - numberRadius()} />
      </Show>
    </>
  );
};
