 ...
 *
 * 目盛線 (showTicks): 各帯のくぎり (中心から縁への放射線) を引いて 1 分ずつ数えられる扇にする。線は帯の
 * 塗りの上・数字の下に乗る (数字の可読性は保つ)。残り扇に付けると、現在針が進んで整数分を越えるたびにその線が
 * 残り扇から外れて消える = 到達済みの分は線なしのグラデ扇に戻る。両端 (現在針・終了マーカー) の線はそれぞれの
 * 針の真下に来るのでほぼ針に隠れ、内側の整数分だけが目盛として見える。
 */
...
const TimerWedge: Component<TimerWedgeProps> = (props) => {
  ...
  const wedge = createMemo(() => {
    ...
    // 各くぎりに 1 本ずつ放射線 (残り扇のみ)。線はすべて中心始点なので縁側の端点だけ持てばよい。
    const ticks = props.showTicks ? edges.map(rimPoint) : [];
    return { bands, ticks };
  });
